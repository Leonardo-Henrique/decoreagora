...
func (uc *LoginUsecase) Login(login models.LoginRequest) error {
...
func (uc *LoginUsecase) AuthenticateCode(req models.AutheticateCodeRequest) (*models.LoginResponse, error) {
	...
	storedCode, err := uc.db.GetAccessCodeByUserID(user.ID)
	if err != nil {
		log.Println(err)
	...
	if err := uc.db.DeleteAccessCode(user.ID, storedCode.Code); err != nil {
